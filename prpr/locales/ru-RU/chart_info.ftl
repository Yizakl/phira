edit-chart = Редактировать параметры
level-displayed = Уровень
chart-name = Имя
author = Автор чарта
composer = Композитор
illustrator = Автор иллюстрации
diff = Сложность
preview-time = Превью трека
preview-hint = Используйте формат [НАЧАЛО] - [КОНЕЦ].
invalid-time = Невозможно использовать это время.
preview-too-short = Слишком короткое превью
preview-too-long = Слишком длинное превью
offset = Задержка
aspect-ratio = Соотношение сторон
ps = Подсказка:
aspect-hint = Соотношение сторон можно указать индексом (например, 1.33333333 для 4:3) или более стандартным способом [ДЛИНА:ШИРИНА]
force-aspect-ratio = Принудительное соотношение сторон
dim = Затенение заднего плана
enable-unlock = Включить видео разблокировки
chart-file = Чарт
music-file = Трек
illu-file = Обложка
unlock-file = Видео
tip = Подсказка
intro = Описание
tags = Теги
tag-exists = Этот тег уже присутствует
illegal-input = Неправильно введены данные
